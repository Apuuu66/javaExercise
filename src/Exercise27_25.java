...
public class Exercise27_25 extends JApplet {
  ...
  private UnweightedGraph<Vertex> graph = null;
  private GraphView view = new GraphView();  
  private AbstractGraph<Vertex>.Tree tree = null;
  
  public static void main(String[] args) {
    JFrame frame = new JFrame();
    JApplet applet = new Exercise27_25();
    frame.add(applet);
    frame.setTitle("Exercise27_25");
  ...
  public Exercise27_25() {
  	jtfStartVertex.setHorizontalAlignment(JTextField.RIGHT);
    JPanel panel1 = new JPanel();
    panel1.add(new JLabel("Starting vertex: "));
    panel1.add(jtfStartVertex);
    panel1.add(jbtDFS);
    panel1.add(jbtBFS);
    panel1.setBorder(new TitledBorder("Display DFS/BFS"));
  ...
  void updateGraph() {
    ArrayList<AbstractGraph.Edge> listOfEdges = new ArrayList<AbstractGraph.Edge>();
    for (int i = 0; i < edges.size(); i++) {
    	int x = list.indexOf(edges.get(i).u);
    	int y = list.indexOf(edges.get(i).v);
    	listOfEdges.add(new AbstractGraph.Edge(x, y));
    	listOfEdges.add(new AbstractGraph.Edge(y, x));
    }
   
    graph = new UnweightedGraph<Vertex>(listOfEdges, list);
  }
  ...
  class GraphView extends JPanel {
    ...
    public void setPath(List<Vertex> path) {
  	...
  	protected void paintComponent(Graphics g) {
  		...
  		for (int i = 0; i < edges.size(); i++) {
  			connectTwoCircles(g, edges.get(i).u.getX(), edges.get(i).u.getY(), edges.get(i).v.getX(), edges.get(i).v.getY(), Vertex.RADIUS);
  		}
