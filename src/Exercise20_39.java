...
public class Exercise20_39 extends JApplet {
  private JTextField jtfDepth = new JTextField(5);
  private TreePanel treePanel = new TreePanel();
  
  public Exercise20_39() {
  ...
  class TreePanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
      ...
      paintBranch(g, depth, startX, startY - 10, getHeight() / 3, Math.PI / 2);
    }
    
    double angleFactor = Math.PI / 5;
    double sizeFactor = 0.58;
    
    public void paintBranch(Graphics g, int depth, int x1, int y1, double length, 
    		double angle) {
      if (depth >= 0) {
    	...
    	paintBranch(g, depth - 1, x2, y2, length * sizeFactor,
    			angle + angleFactor);
  ...
  public static void main(String[] args) {
    JApplet applet = new Exercise20_39();
    applet.init();
    applet.start();
