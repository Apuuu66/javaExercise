...
public class Exercise18_33 extends JApplet {
  private JTextField[][] jtfNumbers = new JTextField[6][7];
  private JButton jbtSolve = new JButton("Solve");
  private int[][] numbers = new int[6][7];
  
  public Exercise18_33() {   
    JPanel panel1 = new JPanel(new GridLayout(6, 7));    
    for (int i = 0; i < jtfNumbers.length; i++) 
      for (int j = 0; j < jtfNumbers[i].length; j++) {
        panel1.add(jtfNumbers[i][j] = new JTextField((int)(Math.random() * 10) + ""));
        jtfNumbers[i][j].setHorizontalAlignment(JTextField.CENTER);
      }
      ...
      public void actionPerformed(ActionEvent e) {
        for (int i = 0; i < numbers.length; i++)
          for (int j = 0; j < numbers[i].length; j++) {
            jtfNumbers[i][j].setBackground(Color.WHITE);
            if (jtfNumbers[i][j].getText().trim().length() == 0) {
              JOptionPane.showMessageDialog(null, "All cells must be filled");
              return;
            }
            else 
              numbers[i][j] = Integer.parseInt(jtfNumbers[i][j].getText().trim());
          }
        
        int[][] result = isConsecutiveFour(numbers);
        if (result != null) {
          jtfNumbers[result[0][0]][result[0][1]].setBackground(Color.YELLOW);
          jtfNumbers[result[1][0]][result[1][1]].setBackground(Color.YELLOW);
          jtfNumbers[result[2][0]][result[2][1]].setBackground(Color.YELLOW);
          jtfNumbers[result[3][0]][result[3][1]].setBackground(Color.YELLOW);
        }
        else {
          JOptionPane.showMessageDialog(null, "No four consecutive numbers in a row, a column, or a diagonal");          
        }
      }
    });
  }
  ...
  public static void main(String[] args) {
    JFrame frame = new JFrame("Exercise18_33");    
    JApplet applet = new Exercise18_33();
    frame.add(applet);
