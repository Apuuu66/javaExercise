...
public class Exercise16_29 extends JFrame {
  int pathLength = 0;
  ...
  class Lattice extends JPanel {
    protected void paintComponent(Graphics g) {     
      ...
      int j = (N + 1) / 2; 
      int count = 0;
      while (lattice[i][j] != null && count < pathLength) {
        Point p = lattice[i][j];
      ...
      if (lattice[i][j] == null) {
        timer.stop();
        pathLength = 0; // Reset pathLength for animating a new path
      }
    }
  }
  
  public static void main(String[] args) {
    JFrame frame = new Exercise16_29();
